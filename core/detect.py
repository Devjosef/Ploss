...
class Detector:
    ...
    def status(self):
        ...
        if not self.baseline_established:
            self.icmp_baseline = statistics.mean(self.icmp_loss_history)
            self.udp_baseline = statistics.mean(self.udp_loss_history)
            self.baseline_established = True
            return (f"ICMP base: {self.icmp_baseline*100:.1f}% "
                   f"UDP base: {self.udp_baseline*100:.1f}% "
                   f"(S:{self.icmp_successes+self.udp_successes}/"
                   f"{len(self.combined_losses)*2})")
        
    recent_var = statistics.variance(list(self.combined_losses) [:-30])
    alert_size = 75 if self.adaptive and recent_var > 0.01 else self.alert_window
    current_losses = list(self.combined_losses)[-alert_size:]
    current_avg = statistics.mean(current_losses)

    icmp_recent = statistics.mean(list(self.icmp_loss_history)[-10:])
    udp_recent = statistics.mean(list(self.udp_loss_history)[-10:])
